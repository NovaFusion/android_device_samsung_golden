...
//TODO should be "OMX.Symbian.Index.Config.Video.SequenceHeader".
 */
typedef struct OMX_SYMBIAN_VIDEO_CONFIG_SEQUENCEHEADERTYPE
{ 
...
} OMX_SYMBIAN_VIDEO_CONFIG_SEQUENCEHEADERTYPE;
...
typedef struct OMX_SYMBIAN_VIDEO_PARAM_AVCSEITYPE
{
    ...
    OMX_U32 eSEI;  /**< bitfield for Requested SEI messages. Several messages can be requested. */
...
/**
Structure pair will be called by the IL Client for N number 
of times where nExtendedLevelIndex ranges from 0 to N-1. 
Both eProfile and nExtendedLevelIndex are input parameters. 
If there are no more supported extended levels for the given profile,
then OMX_GetParamter will return OMX_ErrorNoMore. The caller is 
required to type cast both the eProfile and nExtendedLevel 
parameters to the proper data enumeration types prior to interpreting 
the parameter information. The type casting is to be based on the 
eCompressionFormat parameter defined as per the port definition.
*/
typedef struct OMX_SYMBIAN_VIDEO_PARAM_EXTENDEDPROFILELEVELTYPE {
    OMX_U32 nSize;    
	OMX_VERSIONTYPE nVersion;    
	OMX_U32 nPortIndex;    
	OMX_U32 eProfile;				/**< Input parameter. It is the profile as associated with the eCompressionFormat parameter */
	OMX_U32 nExtendedLevelIndex;	/**< Input parameter. It is used to enumerate the supported extended levels for the given profile */
    OMX_U32 nExtendedLevel;			/**< Output parameter. It is the extended level setting as associated with the eCompressionFormat and eProfile parameters */
    OMX_U32 nFrameWidth;			/**< Output parameter. It is the max frame width associated with the supported extended level. */
    OMX_U32 nFrameHeight;			/**< Output parameter. It is the max frame height associated with the supported extended level. */
    OMX_U32 nMaxBitRate;			/**< Output parameter. It is the max bit rate associated with the supported extended level. */
	OMX_U32 nMaxMBRate;				/**< Output parameter. It is the max MB rate associated with the supported extended level.  */
} OMX_SYMBIAN_VIDEO_PARAM_EXTENDEDPROFILELEVELTYPE;
